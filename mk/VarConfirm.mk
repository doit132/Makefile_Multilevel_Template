...
ALL_INC_PATH += $(shell find $(PROJECT_ABS_ROOT_DIR) -type f -name "*.h" -exec dirname {} \; | sort -u)
EXCLUDE_INC_PATH +=
INC_PATH += $(filter-out $(EXCLUDE_INC_PATH),$(ALL_INC_PATH))

# 函数的作用是将 INC_PATH 变量中的每个元素（目录名）前面加上 -I, 以构建用于指定头文件搜索路径的参数
INCFLAGS := $(patsubst %, -I %, $(INC_PATH))
